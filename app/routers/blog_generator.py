...
async def generate_post_enhanced_gemini_2_flash(req: PostRequest, db: Session = Depends(get_db)):
    ...
    return await generate_post_enhanced(req, db)
        ...
        return PostResponse(
            success=True,
            message="향상된 기능을 포함한 블로그 포스트가 성공적으로 생성되었습니다.",
            data=response_data
        )
...
async def generate_post_enhanced_gemini_2_flash(req: PostRequest, db: Session = Depends(get_db)):
    """
    Gemini 2.0 Flash를 사용한 향상된 블로그 포스트 생성 엔드포인트
    """
    logger.info("=== generate-post-enhanced-gemini-2-flash 엔드포인트 호출됨 ===")
    ...
    try:
        ...
        # 4단계: Gemini 2.0 Flash 향상된 콘텐츠 생성
        logger.info("🤖 4단계: Gemini 2.0 Flash 향상된 콘텐츠 생성")
        try:
            ...
            result = await _create_enhanced_blog_post_with_gemini(
                ...
                keywords=keywords,
                rule_guidelines=req.rules,
                content_length=req.content_length or "5000",
                ai_mode="gemini_2_0_flash"
            )
            logger.info("✅ Gemini 2.0 Flash 향상된 콘텐츠 생성 완료")
        except Exception as e:
            logger.error(f"Gemini 2.0 Flash 향상된 콘텐츠 생성 실패: {e}")
            raise HTTPException(status_code=500, detail=f"콘텐츠 생성 중 오류가 발생했습니다: {str(e)}")
        ...
        logger.info("📊 6단계: API 사용량 기록")
        try:
            increase_api_usage_count("gemini_2_0_flash_enhanced")
            logger.info("✅ API 사용량 기록 완료")
        except Exception as e:
            logger.warning(f"API 사용량 기록 실패: {e}")
        
        logger.info("🎉 Gemini 2.0 Flash 향상된 블로그 포스트 생성 완료!")
        logger.info(f"📈 최종 통계: 제목={db_post.title}, 키워드={db_post.keywords}, 생성시간={db_post.created_at}")
        ...
        response_data = {
            ...
            "source_url": db_post.original_url,
            "created_at": db_post.created_at.isoformat(),
            "ai_mode": "gemini_2_0_flash_enhanced",
            "word_count": result.get('word_count', 0),
            "ai_analysis": result.get('ai_analysis', {}),
            "guidelines_analysis": result.get('guidelines_analysis', {})
        }
        ...
        return PostResponse(
            success=True,
            message="Gemini 2.0 Flash를 사용한 향상된 블로그 포스트가 성공적으로 생성되었습니다.",
            data=response_data
        )
        ...
        logger.error(f"❌ Gemini 2.0 Flash 향상된 블로그 포스트 생성 실패: {e}")
        logger.error(f"🔍 오류 상세: {type(e).__name__}: {str(e)}")
        raise HTTPException(status_code=500, detail=f"블로그 포스트 생성 중 오류가 발생했습니다: {str(e)}")
